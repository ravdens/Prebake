...
def group_stages_by_build_order(stages, unresolved_set):
    """
    Group Docker stages into parallel build groups using Kahn's algorithm (BFS topological level assignment).
    Deterministically produces the minimum number of groups.

    Params:
        - stages: list[DockerStage] list of stages to group by build order
        - unresolved_set: set() set of unresolved dependencies (external base images)
    Returns:
    ...
    # Build in-degree counts and reverse adjacency (dependents map).
    # Only count edges to stages we control (skip unresolved/external images).
    in_degree = {stage.stage_name: 0 for stage in stages}
    dependents = {stage.stage_name: [] for stage in stages}
    ...
    for stage in stages:
        for dep_name in stage.get_all_dependencies():
            ...
            dependents[dep_name].append(stage.stage_name)

    # BFS by levels -- each "wave" of zero-in-degree nodes is one parallel build group
    current_level = sorted([name for name, deg in in_degree.items() if deg == 0])
    groups = []
    visited_count = 0

    while current_level:
        groups.append([name_to_stage[n] for n in current_level])
    ...
    if visited_count != len(stages):
        unvisited = [s.stage_name for s in stages if in_degree[s.stage_name] != 0]
        raise ValueError(f"Circular dependency detected involving: {', '.join(unvisited)}")

    if args.verbose:
        cli_middle("Dependency levels (Kahn's BFS)")
        for idx, group in enumerate(groups):
            cli_info(f"  Level {idx}: {[s.stage_name for s in group]}")
        cli_div()
